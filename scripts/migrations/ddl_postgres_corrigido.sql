...
DECLARE
    dias_atraso INTEGER;
BEGIN
    dias_atraso := DATE(data_devolucao) - data_prevista;
...
DECLARE
    v_id_titulo INT;
BEGIN
    -- Em DELETE, NEW é NULL; usa OLD. Nos demais casos usa NEW.
    IF TG_OP = 'DELETE' THEN
        v_id_titulo := OLD.id_titulo;
    ELSE
        v_id_titulo := NEW.id_titulo;
    END IF;

    UPDATE titulos
    SET qtde = (SELECT COUNT(*) FROM cds WHERE id_titulo = v_id_titulo)
...
AFTER INSERT ON cds
FOR EACH ROW EXECUTE FUNCTION trg_atualizar_qtde_titulo();
...
-- Sistema de Locação de CDs - Migração do VB6 legado
